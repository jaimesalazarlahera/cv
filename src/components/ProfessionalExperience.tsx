...
export default function ProfessionalExperience({ title, experiences, lang }: ProfessionalExperienceProps) {
  ...
  function formatPeople(lang: string, people: number) {
    if (lang === "en") {
      return `Responsible for ${people} ${people === 1 ? "person" : "people"}`;
    } else {
      return `Responsable de ${people} ${people === 1 ? "persona" : "personas"}`;
    }
  }
          ...
          <div key={i}>
            <div className="flex justify-between items-baseline mb-2">
              <div className="flex items-baseline gap-2">
                ...
                {job.frontmatter.people ? <span className="text-sm text-gray-600">({formatPeople(lang, job.frontmatter.people)})</span> : null}
              </div>
